...
RunService.RenderStepped:Connect(function()
	if grabting then
		-- Aquí se detectan los ítems o Brainrot y se "recogen"
		-- Este es un ejemplo genérico, ajustable según el juego:
		for _, item in pairs(workspace:GetChildren()) do
			if item.Name:lower():find("brainrot") then
				if item:IsA("Model") and item:FindFirstChild("TouchInterest") then
					-- Teletransportarse cerca y tocarlo
...
-- KIK
KikBtn.MouseButton1Click:Connect(function()
